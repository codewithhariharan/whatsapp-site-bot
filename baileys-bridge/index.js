...
async function startSock() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const m of messages) {
      ...
      if (!isGroup) continue // groups only — the bot serves no 1:1 chats
      ...
      // ever holds ids we actually acted on. A message with no id can't be
      // deduped, so forward it rather than drop it.
